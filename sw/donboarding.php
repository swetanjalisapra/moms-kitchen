...
include "connect.php";
$id=$_GET['id'];
  $result1 = mysqli_query($conn," SELECT * FROM `delivery` where id=$id") or die( mysqli_error($conn) );
...
if (isset($_POST['cpassq'])) {
	...
	if($p==$cp){
			...
			$query = "UPDATE delivery set pass='$p' where id='$id' ";
		 $sql=mysqli_query($conn, $query);
	             if($sql==true){
					  echo "<script type='text/javascript'>alert('Set Successfully. You can now Login!!');</script>";
					  echo"<script> window.location.href='Delivery/login/login.php'; </script>";
				 }
		...
		<div class="container">
			...
			<div class="contact-agileinfo">
				<div class="col-md-7 contact-right"> 
					<form action="donboarding.php?id=<?php echo $id;?>&h=<?php echo $h;?>" method="post">  
